      ...
      {faqs.map((faq, i) => (
        <div key={i} className="py-5">
          ...
          >
            ...
            {openIndex === i ? <ChevronUp className="mt-1 h-5 w-5 shrink-0 text-[#b86f3c]" /> : <ChevronDown className="mt-1 h-5 w-5 shrink-0 text-[#b86f3c]" />}
          </button>
          {openIndex === i && (
    ...
    <PageShell>
      ...
      <section className="bg-white py-20">
        <div className="container max-w-4xl">
          <p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#b86f3c]">关于 {config.suburb}</p>
      ...
      <section className="bg-white py-24">
        <div className="container grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {["所在 suburb 与街区", "问题类型：漏水、通渠、热水、燃气", "是否紧急或正在返水/喷水", "照片、视频、设备品牌或位置"].map((item) => <article key={item} className="trade-card bg-[#faf8f5] p-6 shadow-sm"><ShieldCheck className="h-6 w-6 text-[#b86f3c]" /><p className="mt-4 font-display text-2xl font-bold leading-tight text-[#173557]">{item}</p></article>)}
          </div>
        </div>
      </section>
