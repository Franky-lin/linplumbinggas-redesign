...
function SectionHeading({ eyebrow, title, copy }: { eyebrow: string; title: string; copy: string }) {
  return (
    <div className="max-w-3xl copper-line pl-5">
      <p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#8a4e20]">{eyebrow}</p>
    ...
    <div className="min-h-screen bg-[#faf8f5] text-[#173557]">
      ...
      <main id="top">
        <section className="relative isolate min-h-[760px] overflow-hidden bg-[#102941] text-white">
          <img src={REAL_PHOTOS.heroVehicleTools} alt="Lin Plumbing and Gas work vehicle and tools in Sydney" className="absolute inset-0 -z-20 h-full w-full object-cover" fetchPriority="high" decoding="async" />
            ...
            <aside className="trade-card hidden bg-[#faf8f5]/95 p-6 text-[#173557] shadow-2xl lg:block">
              ...
              <div className="mt-6 space-y-4">
                {t.trust.slice(0, 3).map((item) => {
                  ...
                  return <div key={item.title} className="flex gap-4 border-l-4 border-[#f5a623] bg-white p-4"><Icon className="mt-1 h-6 w-6 shrink-0 text-[#8a4e20]" /><div><p className="font-display text-xl font-bold">{item.title}</p><p className="text-sm font-semibold text-[#627386]">{item.detail}</p></div></div>;
                })}
              </div>
            </aside>
          </div>
        ...
        <section className="bg-[#faf8f5] py-14">
          <div className="container grid gap-8 border-l-8 border-[#f5a623] bg-white p-6 shadow-sm lg:grid-cols-[0.82fr_1.18fr] lg:items-center lg:p-8">
            <div>
              <p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#8a4e20]">{t.chineseClientEyebrow}</p>
          ...
          <div className="container">
            ...
            <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {t.services.map((service, index) => {
                ...
                return <article key={service.title} className="trade-card overflow-hidden bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl"><img src={service.image} alt={service.title} className="h-44 w-full object-cover object-center" loading="lazy" decoding="async" /><div className="p-6"><div className="flex items-start justify-between gap-5"><div className="flex h-14 w-14 shrink-0 items-center justify-center bg-[#f5a623] text-[#142f4d]"><Icon className="h-7 w-7" /></div><span className="font-display text-sm font-bold text-[#8a4e20]">0{index + 1}</span></div><h3 className="mt-7 font-display text-3xl font-bold leading-none text-[#173557]">{service.title}</h3><p className="mt-4 text-base leading-7 text-[#526476]">{service.text}</p></div></article>;
              })}
            </div>
          </div>
        </section>
        ...
        <section id="gas" className="scroll-mt-32 bg-[#102941] py-24 text-white">
          <div className="container grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div className="grid gap-4 sm:grid-cols-2"><img src={REAL_PHOTOS.gasToolsSite} alt="Gas service tools" className="h-72 w-full object-cover object-center" loading="lazy" decoding="async" /><img src={REAL_PHOTOS.gasMeterTest} alt="Gas meter work" className="h-72 w-full object-cover object-center" loading="lazy" decoding="async" /><img src={REAL_PHOTOS.gasPipeWall} alt="Gas pipework" className="h-56 w-full object-cover object-center sm:col-span-2" loading="lazy" decoding="async" /></div>
        ...
        <section className="py-24"><div className="container grid gap-10 lg:grid-cols-[1fr_1fr] lg:items-center"><div><SectionHeading eyebrow={t.drainsEyebrow} title={t.drainsTitle} copy={t.drainsCopy} /><div className="mt-8"><CallButtons t={t} /></div></div><div className="grid gap-4 sm:grid-cols-2"><img src={REAL_PHOTOS.cctvDrainInspection} alt="CCTV pipe inspection equipment" className="h-80 w-full object-cover object-center" loading="lazy" decoding="async" /><img src={REAL_PHOTOS.stormwaterDrainage} alt="Stormwater drainage" className="h-80 w-full object-cover object-center" loading="lazy" decoding="async" /></div></div></section>

        <section id="projects" className="scroll-mt-32 bg-[#173557] py-24 text-white"><div className="container"><div className="max-w-3xl"><p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#f5a623]">{t.projectsEyebrow}</p><h2 className="mt-3 font-display text-5xl font-bold leading-none md:text-7xl">{t.projectsTitle}</h2><p className="mt-5 text-lg leading-8 text-white/72">{t.projectsCopy}</p></div><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{projectImages.map((src, index) => <figure key={src} className="group relative h-80 overflow-hidden bg-[#071526]"><img src={src} alt={t.projectLabels[index]} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" decoding="async" /><figcaption className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#071526] to-transparent p-5"><p className="font-display text-2xl font-bold">{t.projectLabels[index]}</p></figcaption></figure>)}</div></div></section>

        <section className="engineering-paper py-24"><div className="container grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center"><img src={REAL_PHOTOS.teamVehicleTrust} alt="Lin Plumbing and Gas service vehicle" className="h-[520px] w-full object-cover object-center shadow-2xl" loading="lazy" decoding="async" /><div><SectionHeading eyebrow={t.areasEyebrow} title={t.areasTitle} copy={t.areasCopy} /><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{t.areaTags.map((tag) => <p key={tag} className="border-l-4 border-[#f5a623] bg-white p-4 font-bold">{tag}</p>)}</div></div></div></section>

        <section className="bg-[#faf8f5] py-24"><div className="container"><div className="grid gap-10 lg:grid-cols-[0.75fr_1.25fr]"><div><p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#8a4e20]">{t.seoEyebrow}</p><h2 className="mt-3 font-display text-5xl font-bold leading-none text-[#173557] md:text-7xl">{t.seoTitle}</h2><p className="mt-6 text-lg leading-8 text-[#526476]">{t.seoCopy}</p></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{t.seoPoints.map((point) => <article key={point.title} className="trade-card border-t-4 border-[#f5a623] bg-white p-6 shadow-sm"><h3 className="font-display text-2xl font-bold leading-tight text-[#173557]">{point.title}</h3><p className="mt-4 text-base leading-7 text-[#526476]">{point.text}</p></article>)}</div></div></div></section>

        <section className="engineering-paper py-24"><div className="container grid gap-10 lg:grid-cols-[0.82fr_1.18fr]"><div><p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#8a4e20]">{t.faqEyebrow}</p><h2 className="mt-3 font-display text-5xl font-bold leading-none text-[#173557] md:text-7xl">{t.faqTitle}</h2><p className="mt-6 text-lg leading-8 text-[#526476]">{t.faqCopy}</p></div><div className="space-y-4">{t.faqs.map((faq) => <article key={faq.question} className="trade-card bg-white p-6 shadow-sm"><h3 className="font-display text-2xl font-bold leading-tight text-[#173557]">{faq.question}</h3><p className="mt-3 text-base leading-7 text-[#526476]">{faq.answer}</p></article>)}</div></div></section>

        <section id="contact" className="scroll-mt-32 bg-[#102941] py-24 text-white"><div className="container grid gap-10 lg:grid-cols-[0.85fr_1.15fr]"><div><p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#f5a623]">{t.contactEyebrow}</p><h2 className="mt-3 font-display text-5xl font-bold leading-none md:text-7xl">{t.contactTitle}</h2><div className="mt-8 space-y-4 text-lg"><a href={BUSINESS.phoneHref} className="flex items-center gap-3 hover:text-[#f5a623]"><Phone className="h-5 w-5" /> {BUSINESS.phone}</a><a href={BUSINESS.emailHref} className="flex items-center gap-3 hover:text-[#f5a623]"><Mail className="h-5 w-5" /> {BUSINESS.email}</a><p className="flex items-center gap-3"><MapPin className="h-5 w-5" /> {BUSINESS.serviceArea}</p><p>{BUSINESS.license} · {BUSINESS.abn}</p></div></div><form onSubmit={(e) => { e.preventDefault(); const form = e.currentTarget; const data = new FormData(form); const subject = encodeURIComponent('Quote Request - ' + (data.get('service') || 'Plumbing Enquiry')); const body = encodeURIComponent(`Name: ${data.get('name')}\nPhone: ${data.get('phone')}\nService: ${data.get('service')}\nMessage: ${data.get('message')}`); window.location.href = `mailto:FRANKY@LINPLUMBINGGAS.COM?subject=${subject}&body=${body}`; }} className="trade-card bg-[#faf8f5] p-6 text-[#173557]"><div className="grid gap-4 sm:grid-cols-2"><label className="font-bold">{t.form.name}<input name="name" className="mt-2 w-full border border-[#173557]/20 bg-white p-3 font-normal" placeholder={t.form.namePlaceholder} /></label><label className="font-bold">{t.form.phone}<input name="phone" className="mt-2 w-full border border-[#173557]/20 bg-white p-3 font-normal" placeholder={t.form.phonePlaceholder} /></label></div><label className="mt-4 block font-bold">{t.form.service}<input name="service" className="mt-2 w-full border border-[#173557]/20 bg-white p-3 font-normal" placeholder={t.form.servicePlaceholder} /></label><label className="mt-4 block font-bold">{t.form.message}<textarea name="message" rows={5} className="mt-2 w-full border border-[#173557]/20 bg-white p-3 font-normal" placeholder={t.form.messagePlaceholder} /></label><p className="mt-4 text-sm text-[#637083]">{t.form.note}</p><Button type="submit" className="mt-5 h-13 w-full rounded-none bg-[#f5a623] text-base font-extrabold text-[#142f4d] hover:bg-[#ffc14d]">{t.form.submit}</Button></form></div></section>
      </main>
