    ...
    <PageShell>
          ...
          <aside className="trade-card hidden bg-[#faf8f5]/95 p-6 text-[#173557] shadow-2xl lg:block">
            ...
            <div className="mt-6 space-y-3">
              {config.trust.map((item, index) => {
                ...
                return <div key={item} className="flex gap-3 border-l-4 border-[#f5a623] bg-white p-4"><TrustIcon className="mt-1 h-5 w-5 shrink-0 text-[#8a4e20]" /><p className="font-bold">{item}</p></div>;
              })}
            </div>
          </aside>
        </div>
      ...
      <section className="engineering-paper py-24">
        <div className="container grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {config.symptoms.map((symptom) => <article key={symptom} className="trade-card border-l-4 border-[#f5a623] bg-white p-5 shadow-sm"><div className="flex gap-3"><AlertTriangle className="mt-1 h-5 w-5 shrink-0 text-[#8a4e20]" /><p className="font-bold leading-7 text-[#173557]">{symptom}</p></div></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#102941] py-24 text-white">
        <div className="container grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div className="grid gap-4 sm:grid-cols-2">
            <img src={config.relatedImage} alt={config.relatedAlt} className="h-80 w-full object-cover object-center" loading="lazy" decoding="async" />
            <img src={REAL_PHOTOS.teamVehicleTrust} alt="Lin Plumbing and Gas 悲尼服务车辆" className="h-80 w-full object-cover object-center" loading="lazy" decoding="async" />
          </div>
          <div>
      ...
      <section className="bg-[#faf8f5] py-24">
        <div className="container grid gap-10 lg:grid-cols-[0.72fr_1.28fr]">
          <div>
            <p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#8a4e20]">服务区域</p>
      ...
      <section className="engineering-paper py-24">
        <div className="container grid gap-10 lg:grid-cols-[0.75fr_1.25fr]">
          <div>
            <p className="font-display text-sm font-bold uppercase tracking-[0.24em] text-[#8a4e20]">相关中文服务</p>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {chineseServiceLinks.filter((link) => link.href !== config.meta.path).map((link) => <a key={link.href} href={link.href} className="trade-card overflow-hidden bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><img src={link.image} alt={link.title} className="h-44 w-full object-cover object-center" loading="lazy" decoding="async" /><div className="p-5"><h3 className="font-display text-2xl font-bold leading-tight text-[#173557]">{link.title}</h3><p className="mt-3 leading-7 text-[#526476]">{link.text}</p></div></a>)}
          </div>
        </div>
      </section>
